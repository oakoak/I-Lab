...
{
    return ((stek_ok) && (stek_ok->counter >= 0) && (stek_ok->max_size > stek_ok->counter) && ((stek_ok->type == sizeof(double)) || (stek_ok->type == sizeof(char))));
}
...
int push (struct stek* stek_push, double x)
{
    ...
    if (stek_push->type == sizeof(char))
    {
        stek_push->data[stek_push->counter++] = (char) x;
        return 0;
    }

    *((double*) &stek_push->data[stek_push->counter]) = x;
    stek_push->counter+=sizeof(double);
...
double stack_pop(struct stek* stek_pop)
{
    ...
    if (stek_pop->type == sizeof(char))
        return stek_pop->data[--stek_pop->counter];
    else
        return *((double*) &stek_pop->data[stek_pop->counter-=sizeof(double)]);
}
